...
class _UpdatePasswordScreenState extends State<UpdatePasswordScreen> {
  ...
  @override Widget build(BuildContext context)=>MediReservaPage(title:'Nueva contraseña',step:'',child:Form(key:_key,child:Column(children:[
    const Text('Define una nueva contraseña para tu cuenta.',textAlign:TextAlign.center,style:TextStyle(color:kMediMuted,fontSize:8.5)),const SizedBox(height:14),
    TextFormField(controller:_p,obscureText:true,validator:(v)=>v==null||v.length<6?'Mínimo 6 caracteres':null,decoration:const InputDecoration(labelText:'Nueva contraseña',border:OutlineInputBorder())),const SizedBox(height:9),
    ...
    if(_error!=null)Padding(padding:const EdgeInsets.only(top:9),child:Text(_error!,style:const TextStyle(color:Colors.red,fontSize:8))),const SizedBox(height:12),MediButton(label:_busy?'Guardando...':'Guardar contraseña',onPressed:_busy?null:_save)
  ])));
}
